...
if [ ! -d "${KYROS_DIR}/src" ]; then
    echo "--> Cloning Nikto..."
    git clone "${REPO}" "${KYROS_DIR}"
fi
...
uv pip install --system -e packages/kyros-core
uv pip install --system -e packages/kyros-cli
