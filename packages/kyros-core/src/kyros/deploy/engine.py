    ...
    def _do_deploy(self, record: DeploymentRecord, config: dict) -> bool:
        if record.target == DeploymentTarget.LOCAL:
            ...
            try:
                ...
                sftp.put(config.get("package_path", ""), f"{remote_dir}/kyros.tar.gz")
                ssh.exec_command(f"cd {remote_dir} && tar -xzf kyros.tar.gz && python3 install.py")
                ssh.close()
                return True
            except ImportError:
                return False
...
DEPLOYMENT_TEMPLATES = {
    "linux_server": {"components": ["kyros-core", "nikto-web", "nginx"], "min_ram": 2},
    "raspberry_pi": {"components": ["kyros-core"], "min_ram": 1, "arch": "arm64"},
    "docker": {"components": ["kyros-core", "nikto-web"], "image": "nikto:latest"},
    "kubernetes": {"components": ["kyros-core", "nikto-web", "redis"], "replicas": 3},
