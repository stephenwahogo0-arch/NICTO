"""KYROS Registration system — first-time onboarding with identity verification, consent, and emergency contact."""
import getpass
import hashlib
import json
import os
...
class RegistrationFlow:
    ...
    def _collect_info(self, defaults: Optional[dict] = None) -> RegistrationData:
        ...
        console.print(Panel.fit(
            "[bold cyan]KYROS — First-Time Registration[/]\n\n"
            "[white]Before you can use KYROS, we need some information for:\n"
        ...
        if not Confirm.ask(
            ...
            console.print("[red]You must accept the Privacy Policy to use KYROS.[/]")
            console.print("[red]Exiting registration. Run 'kyros' again when ready.[/]")
            raise SystemExit(0)
        ...
        if not age_confirmed:
            console.print("[red]KYROS is not available to users under 18. Exiting.[/]")
            raise SystemExit(0)
        ...
        console.print("[dim]A PIN to lock KYROS and prevent unauthorized access.[/]")
        ...
        if not Confirm.ask("Is this information correct?", default=True):
            console.print("[yellow]Registration cancelled. Run 'kyros' to try again.[/]")
            raise SystemExit(0)
