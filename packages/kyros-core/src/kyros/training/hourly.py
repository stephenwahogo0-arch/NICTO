...
class HourlyTrainer:
    """Trains KYROS every hour by consolidating conversation history into
    persistent memory. Never forgets what it learned."""

    def __init__(self, data_dir: Optional[str] = None):
        self.data_dir = Path(data_dir or "~/.kyros/training").expanduser()
    ...
    def _classify_topic(self, text: str) -> str:
        ...
        if any(w in text_lower for w in ["brain", "think", "learn", "memory"]):
            return "brain"
        if any(w in text_lower for w in ["who are you", "what are you", "kyros"]):
            return "about_nikto"
    ...
    def get_context_for_query(self, query: str) -> str:
        ...
        if topic in self._learned and self._learned[topic]["patterns"]:
            ...
            context = f"[KYROS learned from {learned['count']} past {topic} conversations]\n"
