...
def create_variant(variant_name: str, config: ModelConfig = None) -> AgentVariant:
    if variant_name == "kyros-heavyweight":
        return AgentVariant("kyros-heavyweight", config or HEAVYWEIGHT_CONFIG)
    elif variant_name == "kyros-sonnet":
        return AgentVariant("kyros-sonnet", config or SONNET_CONFIG)
    elif variant_name == "kyros-mythos":
        return AgentVariant("kyros-mythos", config or MYTHOS_CONFIG)
