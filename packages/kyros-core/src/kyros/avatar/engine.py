...
from kyros.avatar.sprites import AVAILABLE_POSES, AVAILABLE_EXPRESSIONS
from kyros.avatar.animations import AnimationPlayer, AnimationType, Expression
try:
    from kyros.avatar.qt_renderer import QtAvatarRenderer
    _HAS_QT = True
except ImportError:
    _HAS_QT = False
    from kyros.avatar.renderer import AvatarRenderer as QtAvatarRenderer
    import warnings
...
from kyros.avatar.personalize import PersonalAvatarGenerator, REF_DIR


class AvatarEngine:
    """KYROS's headless avatar — moves, talks, animates, controls desktop, sees via webcam."""
    ...
    def generate_lively_frame(self, pose: str = "idle", expression: str = "neutral",
        ...
        if not self.personalizer.has_references():
            from kyros.avatar.sprites import create_avatar_frame
