"""KYROS Core Engine — Rust ECS backend with Python bridge.

This module wraps the compiled native `kyros_core_engine.pyd`/`.so`
and provides a Pythonic interface. If the native binary is not yet
compiled, it raises a clear build instruction.

Usage:
    from kyros.core_engine import kyros ngine
    engine = NiktoEngine()
...
Build instructions:
    ...
    cargo build --release
    cp target/release/kyros_core_engine.pyd ../kyros-core/src/nikto/core_engine/
...
class NiktoEngine:
    ...
    def __init__(self, chunk_size: int = 16, generate_mesh: bool = True):
        if not HAS_NATIVE:
            raise RuntimeError(
                "KYROS Core Engine native module not found.\n\n"
                "Build it:\n"
                "  cd packages/kyros-core-engine\n"
                "  cargo build --release\n"
                "  cp target/release/kyros_core_engine.pyd packages/kyros-core/src/nikto/core_engine/\n\n"
