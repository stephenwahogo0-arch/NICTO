...
class NiktoReadOwnTool(Tool):
    name = "nikto_read_own"
    description = "Read NIKTO's own source code files"

    async def execute(self, path: str = None, package: str = "nikto", **kwargs) -> dict:
        import nikto
...
class NiktoWriteOwnTool(Tool):
    name = "nikto_write_own"
    description = "Write/modify NIKTO's own source code"
...
class NiktoSelfReviewTool(Tool):
    name = "nikto_self_review"
    description = "Review NIKTO's own code quality"
...
async def tool_nikto_read_own(path: str = None) -> dict:
    tool = NiktoReadOwnTool()
    return await tool.execute(path=path)


async def tool_nikto_write_own(path: str, content: str) -> dict:
    tool = NiktoWriteOwnTool()
    return await tool.execute(path=path, content=content)


async def tool_nikto_self_review(path: str = None) -> dict:
    tool = NiktoSelfReviewTool()
    return await tool.execute(path=path)
