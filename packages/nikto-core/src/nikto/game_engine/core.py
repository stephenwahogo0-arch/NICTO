...
"""NIKTO Core Engine — ECS-based real-time game engine with physics, rendering, audio.
...
class NIKTOCoreEngine:
    def __init__(self, title="NIKTO Game", width=800, height=600, fps=60, fullscreen=False, vsync=True):
        self.title = title
        self.width = width
        self.height = height
        self.target_fps = fps
    ...
    def _render(self):
        ...
        if self.debug_mode and self.font_small:
            y = 5
            for line in [
                f"NIKTO Engine | FPS: {self._current_fps}",
