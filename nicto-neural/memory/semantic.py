"""Semantic memory — facts and knowledge, searchable by content."""
...
class SemanticMemory(MemoryStore):
    """Facts and knowledge with content-based search."""

    def __init__(self, db_path):
        path = Path(db_path).expanduser()
        path.mkdir(parents=True, exist_ok=True)
        self._db = sqlite3.connect(str(path / "semantic.db"))
        self._init_schema()
    ...
    def recall(self, query: str, limit: int = 10) -> list:
        cursor = self._db.execute(
            ...
            FROM facts WHERE content LIKE ? ORDER BY importance DESC LIMIT ?""",
            (f"%{query}%", limit),
        )
        results = []
    ...
    def get(self, id: str) -> Optional[MemoryEntry]:
        cursor = self._db.execute(
            "SELECT id, content, metadata, timestamp, importance FROM facts WHERE id=?",
            (id,),
        )
        row = cursor.fetchone()
        if not row:
    ...
    def delete(self, id: str) -> bool:
        self._db.execute("DELETE FROM facts WHERE id=?", (id,))
        self._db.commit()
        return True
