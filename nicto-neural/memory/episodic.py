"""Episodic memory — sequential event memory with timestamps."""
...
class EpisodicMemory(MemoryStore):
    """Sequential event memory. Stores every interaction as an episode."""

    def __init__(self, db_path):
        path = Path(db_path).expanduser()
        path.mkdir(parents=True, exist_ok=True)
        self._db = sqlite3.connect(str(path / "episodic.db"))
        self._init_schema()
    ...
    def get(self, id: str) -> Optional[MemoryEntry]:
        cursor = self._db.execute(
            "SELECT id, content, metadata, timestamp, importance FROM episodes WHERE id=?",
            (id,),
        )
        row = cursor.fetchone()
        if not row:
    ...
    def delete(self, id: str) -> bool:
        self._db.execute("DELETE FROM episodes WHERE id=?", (id,))
        self._db.commit()
        return True
